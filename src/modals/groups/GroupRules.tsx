...
export default function GroupRules({ ruleModeState, rulesState }: GroupRulesProps) {
  ...
  const findRuleByName = useCallback(
    (name: PlayerGroupRuleName): PlayerGroupRuleOption => {
  ...
  const updateRule = useCallback(
    (ruleIdx: number, partial: Partial<UnpackedGroupRule>) => {
      setRules((rules) => {
  ...
  const updateRuleName = useCallback(
    (ruleIdx: number, fieldName: string) => {
      setRules((rules) => {
        return rules.map((rule, idx) => {
          if (idx === ruleIdx) {
  ...
  const getFieldDisplayNameForRule = useCallback(
    (rule: UnpackedGroupRule) => {
  ...
  return (
    <div className='space-y-4 rounded border border-gray-200 p-4 md:max-h-80 md:overflow-y-scroll'>
      <p className='text-sm'>All players</p>

      <div className='space-y-4 divide-y divide-gray-200'>
        {rules.map((rule, idx) => (
          <div key={idx} className='space-y-2 pb-4 text-sm'>
            <div className='flex items-center space-x-2'>
              <div className='h-2 w-3 -translate-y-0.5 border-b border-l border-gray-300' />
              ...
              {rule.namespaced &&
                !metaGroupFields.some((f) => f.metaProp === rule.namespacedValue) && (
                  <TextInput
                    id='namespaced-value'
                    variant='modal'
            ...
            <div className='ml-5 flex items-center space-x-2'>
              <DropdownMenu
                options={availableRules
                  .filter((availableRule) => availableRule.castTypes.includes(rule.castType))
                  .map(({ name, negate }) => ({
