...
export default function DataExports() {
  ...
  const {
    ...
    error: fetchError,
  } = useDataExports(activeGame, createdExportId)
  ...
  const onCreateClick = async (e: MouseEvent<HTMLElement>) => {
    ...
    try {
      ...
      if ((err as AxiosError).response?.status === 402) {
        setCreateError(
          buildError(
            'You have reached the data export limit for this month, please contact the account owner about upgrading the pricing plan',
          ),
        )
      } else {
  ...
  return (
    <Page title='Data exports' isLoading={dataExportsLoading}>
      {!user.emailConfirmed && (
        <AlertBanner
          className='lg:w-max'
          text='You need to confirm your email address to export data'
        />
      )}
      ...
      {!createdExportId && (
        <form className='lg:2/3 w-full space-y-4 xl:w-1/2'>
          <h2 className='text-xl font-bold lg:text-2xl'>Create data export</h2>
          ...
          {entitiesError && (
            <ErrorMessage error={{ message: "Couldn't fetch available entities" }} />
          )}
      ...
      {createdExportId && (
        <div className='lg:2/3 w-full xl:w-1/2'>
          <AlertBanner
            ...
            icon={IconCheck}
            text={"Success! We'll email you when your export is ready"}
          />
        </div>
      )}
    </Page>
